...
'Cadiz', 'Cantabria', 'Castello-Castellon', 'Cordoba', 'Cuenca', 'Girona', 'Granada', 'Guadalajara', 'Guipuzcoa', 'Huelva', 'Huesca', 'Islas-Baleares', 'Jaen', 'La-Rioja', 'Las-Palmas', 'Leon', 'Lleida', 'Lugo', 'Madrid', 'Malaga', 'Murcia', 'Navarra', 'Ourense', 'Palencia', 'Pontevedra', 'Salamanca', 'Santa-Cruz-de-Tenerife', 'Segovia', 'Sevilla', 'Soria', 'Tarragona', 'Teruel', 'Toledo', 'Valencia', 'Valladolid', 'Vizcaya', 'Zamora', 'Zaragoza']
        for province in list_spain_province:
               ...
               #base_url = 'https://es.wallapop.com/rest/seoitems?path=%2Flist%2Fprovincia-Zaragoza%2Fcoches&total=21499&_p='  #finished province one by one
               #base_url = 'https://es.wallapop.com/rest/seoitems?path=%2Flist%2Fcoches&total=102609&_p='# try the hole list
               base_url = 'https://es.wallapop.com/rest/seoitems?path=%2Flist%2Fprovincia-Zaragoza%2Fmotor-y-accesorios&total=595266545&_p='#&lastItemId=165179565
               self.url = base_url + str(0)
               yield Request(self.url, self.parse) #try callback = self.parse
               
    #def find_between( s, first, last ): #use this function if you want to crawl with userId
         #   try:
    ...
    #def parse_user(self, response):  #loop those url 'https://es.wallapop.com/rest/useritems?statuses=sold_outside&userId=56953269&total=969&_p='  and published and userId
       # data = json.loads(response.body)
        #for jsonresponse in data.get('items', []):
                #item["description"] = jsonresponse.get('description')
                #item["carburant"] = find_between(item["description"],'Motor:', 'Cambio:') #to complete the rest of the items...
    ...
    def parse(self, response):
		...
		try:
		        ...
		        item["description"] = jsonresponse.get('description') 
		        item["date_publication"] = jsonresponse.get('publishDateStr') #exist on coches per province
		        #item["response_url"] = response.url #added 19/07
                ...
                try: 
	             	...
	             	item["contact_phone"] = re.findall(r"\D(\d{9})\D", desc)  
        ...
        #new_url = 'https://es.wallapop.com/rest/seoitems?path=%2Flist%2Fprovincia-'+province+'%2Fcoches&total=21499&_p='+str(i)
        yield scrapy.Request(self.url +  str(next_page) + '&lastItemId=' + str(last_item_id) ) #change elf.url
