...
class wallapopSpider(scrapy.Spider):
	...
	def parse(self, response):
                ...
                container = response.css('div.card.js-masonry-item.card-product')
                for link in container:
                      ...
                      yield request  
                hrefs = response.css('div.card.js-masonry-item.card-product > a ::attr(href)').extract()
		...
		last_item_id = last_id[-1]
                ...
                #while True:
                        #next_page = 'https://es.wallapop.com/list/coches?_p='+str(number_next_page)+'&lastItemId='+str(last_item_id)
                        #yield scrapy.Request(next_page) 
                        #if number_current_page == 3223:
                                #break     
                next_page = 'https://es.wallapop.com/list/coches?_p='+str(number_next_page)+'&lastItemId='+str(last_item_id)
                yield scrapy.Request(next_page)        
                           
                #or try to set p=0 and lastitemid=0 for the start_url
                #if the problem persist at the page 54th try to change it to the start url 
